...
async function listHearings(ownerUserId) {
...
async function createHearing(payload, ownerUserId) {
  const { caseId, hearingDateTime, courtroom, agenda, outcome, reminderSent } = payload;
  const [result] = await pool.query(
    `INSERT INTO hearings (owner_user_id, case_id, hearing_date_time, courtroom, agenda, outcome, reminder_sent)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [ownerUserId, caseId, hearingDateTime, courtroom, agenda, outcome || null, reminderSent ? 1 : 0]
  );
